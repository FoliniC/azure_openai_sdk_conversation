...
class ConversationMemoryManager(IMemoryManager):
    ...
    def __init__(
        ...
        self._preserve_system = getattr(
            config, 
            "sliding_window_preserve_system", 
            True
        )
    ...
    async def async_set_base_tool_tokens(self, count: int) -> None:
        ...
        async with self._lock:
            ...
            self._logger.info("Base tool token count set to %d. Updated %d existing windows.", count, len(self._windows))
    ...
    async def async_set_system_prompt(self, conversation_id: str, content: str) -> None:
        ...
        async with self._lock:
            ...
            if existing_prompt_idx != -1:
                ...
                window.messages[existing_prompt_idx].timestamp = datetime.now(timezone.utc)
                window.current_tokens += token_delta
                
                self._logger.debug(
                    "Updated system prompt for conv=%s. Token delta: %d, new total: %d",
                    conversation_id, token_delta, window.current_tokens
                )
            else:
                ...
                self._logger.debug(
                    ...
                    conversation_id, new_token_count, window.current_tokens
                )
    ...
    def _count_tokens(self, text: str) -> int:
        ...
        if not self._encoding:
            self._logger.error("Tiktoken encoding not initialized. Cannot count tokens.")
        ...
        try:
            ...
            self._logger.warning(
                "tiktoken failed, using fallback: %r", err
            )
