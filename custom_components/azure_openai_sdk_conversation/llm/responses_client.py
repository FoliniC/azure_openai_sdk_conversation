...
class ResponsesClient:
    ...
    async def complete(
        ...
        while True:
            attempt_key = f"{self._api_version}::{current_token_param}::{current_format}"
            ...
            try:
                ...
                if "Unsupported parameter" in error_body:
                    if "max_output_tokens" in error_body and current_token_param != "max_completion_tokens":
                        self._logger.debug("Retrying with max_completion_tokens")
                        current_token_param = "max_completion_tokens"
                        continue
                    elif "max_completion_tokens" in error_body and current_token_param != "max_output_tokens":
