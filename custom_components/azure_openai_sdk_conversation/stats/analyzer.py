...
class StatsAnalyzer:
    ...
    def _analyze_performance(self, stats: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if first_chunk_times:
            perf["avg_first_chunk_time_ms"] = sum(first_chunk_times) / len(first_chunk_times)
    ...
    def _analyze_costs(self, stats: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        llm_requests = sum(s['llm_chat_count'] + s['llm_responses_count'] for s in stats)
        ...
        return {
            ...
            "avg_tokens_per_request": total_tokens / llm_requests if llm_requests > 0 else 0.0,
            "total_cost_usd": total_cost,
            "avg_cost_per_request_usd": total_cost / llm_requests if llm_requests > 0 else 0.0,
            "projected_monthly_cost_usd": (total_cost / len(stats)) * 24 * 30,  # Rough estimate
        }
    ...
    def _analyze_errors(self, stats: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate_percentage": (total_errors / total_requests * 100) if total_requests > 0 else 0.0,
    ...
    def _format_text(self, analysis: Dict[str, Any]) -> str:
        ...
        if "avg_first_chunk_time_ms" in perf:
            lines.append(f"Avg First Chunk:     {perf['avg_first_chunk_time_ms']:.1f}ms")
        ...
        if errors.get("error_breakdown"):
            lines.append("\nError Breakdown:")
            for err_type, count in sorted(errors["error_breakdown"].items(), key=lambda x: x[1], reverse=True):
    ...
    def compare_with_baseline(self, baseline_file: Path, hours: int = 24) -> Dict[str, Any]:
        ...
        # Calculate deltas
        for metric in ["avg_execution_time_ms", "total_cost_usd", "error_rate_percentage"]:
            sections = {
            ...
            comparison["delta"][metric] = {
                ...
                "improved": delta < 0 if "error" in metric or "time" in metric or "cost" in metric else delta > 0
            }
