...
"""
    ...
    def get_delta_prompt(
        ...
        if conversation_id not in self._conversations:
            _LOGGER.warning("MCP: Conversation %s not found, should use initial prompt", conversation_id)
            return None
        ...
        # Formatta il delta
        delta_parts = [f"**State Update** (Delta at {datetime.now(timezone.utc).isoformat()}):"]
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_entities_tracked": sum(len(states) for states in self._conversations.values()),
...
def patch_conversation_agent(agent: Any, hass: Any) -> Any:
    ...
    def _mcp_prepare_system_message(self, *args, **kwargs):  
        ...
        if isinstance(prep_result, tuple):  
            ...
            sys_msg = prep_result.get("system_message") or prep_result.get("message") or ""  
