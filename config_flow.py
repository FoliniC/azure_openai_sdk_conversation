...
MODELS_API_VERSION = "2024-02-15-preview"  
...
async def get_sampling_capabilities(  
    ...
    client = AsyncAzureOpenAI(  
        api_key=api_key,  
        api_version="2024-10-01-preview",  # Usiamo una versione recente per capabilities  
        azure_endpoint=root,  
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> None:  
    ...
    payload = {  
        "messages": [  
            {"role": "developer", "content": [{"type": "text", "text": "ping"}]},  
        ],  
        "max_completion_tokens": 10,  
    }  
    extra_body = {_token_param(): payload["max_completion_tokens"]}  
...
class AzureOpenAIConfigFlow(ConfigFlow, domain=DOMAIN):  
    ...
    async def async_step_user(  
        ...
        errors: dict[str, str] = {}  
        try:  
            await validate_input(self.hass, inp)  
        except openai.AuthenticationError:  
    ...
    def _create_entry(self, *, options: Mapping[str, Any]) -> ConfigFlowResult:  
        ...
        return self.async_create_entry(  
            ...
            data={  
                ...
                CONF_CHAT_MODEL: self._step1_data[CONF_CHAT_MODEL],  # <<< MOD  
            },  
            options=base_opts,  
        )  
