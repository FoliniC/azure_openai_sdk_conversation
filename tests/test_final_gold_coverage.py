...
async def test_config_flow_reconfigure_global(hass):
    ...
    assert (
        flow.async_update_reload_and_abort.call_args[1]["data"][CONF_LOGGING_ENABLED]
        is True
    )
...
async def test_config_flow_reconfigure_controller(hass):
    ...
    entry.data = {
        ...
        CONF_TARGET_ENTITY: "climate.old",
    }
    hass.config_entries.async_get_entry = MagicMock(return_value=entry)
    ...
    result = await flow.async_step_reconfigure(
        user_input={CONF_NAME: "New Name", CONF_TARGET_ENTITY: "climate.new"}
    )
    assert result["type"] == "abort"
...
async def test_apply_now_edge_cases(hass):
    ...
    ps.get_profile_data = AsyncMock(
        return_value={"schedule": [{"time": "bad", "value": 20}]}
    )
