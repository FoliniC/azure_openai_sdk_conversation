...
async def test_storage_list_profiles(hass):
    ...
    with patch("pathlib.Path.glob") as mock_glob:
        ...
        manager.load_profile_cached = AsyncMock(
            return_value={"meta": {"preset_type": "thermostat"}}
        )
...
async def test_storage_save_profile(hass):
    ...
    with patch("pathlib.Path.write_text") as mock_write:
        success = await manager.save_profile(
            "NewProfile", "thermostat", {"schedule": []}, {"meta": {}}, "prefix"
        )
...
async def test_storage_backups(hass):
    ...
    manager = StorageManager(
        hass, hass.config.path("cronostar/profiles"), enable_backups=True
    )
...
async def test_load_all_profiles(hass):
    ...
    with patch("pathlib.Path.glob") as mock_glob:
        ...
        with patch.dict(
            sys.modules,
            {"custom_components.cronostar.deep_checks.file_checker": mock_checker_mod},
        ):
