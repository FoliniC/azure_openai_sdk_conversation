...
async def test_async_setup(hass):
    """Test YAML setup."""
    with patch(
        "custom_components.cronostar.async_setup_integration", return_value=True
    ):
...
async def test_async_setup_entry_global_failure(hass):
    ...
    # Global setup fails
    with patch(
        "custom_components.cronostar.async_setup_integration", return_value=False
    ):
        success = await async_setup_entry(hass, entry)
...
async def test_async_setup_entry_controller_lazy_init(hass):
    ...
    with patch(
        ...
        with patch(
            "custom_components.cronostar.CronoStarCoordinator"
        ) as mock_coord_cls:
            mock_coord = mock_coord_cls.return_value
            mock_coord.async_initialize = AsyncMock()
            mock_coord.async_config_entry_first_refresh = AsyncMock()
...
async def test_async_unload_entry(hass):
    ...
    with patch(
        "custom_components.cronostar.setup.services.async_unload_services",
...
async def test_async_reload_entry(hass):
    ...
    with (
        patch(
            "custom_components.cronostar.async_unload_entry", return_value=True
        ) as mock_unload,
        patch(
            "custom_components.cronostar.async_setup_entry", return_value=True
        ) as mock_setup,
    ):
