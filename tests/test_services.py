...
async def test_save_profile_metadata_only(hass, profile_service, mock_storage_manager):
    ...
    profile_service.get_profile_data = AsyncMock(
        return_value={"schedule": [{"time": "12:00", "value": 20.0}]}
    )
    hass.config_entries.async_entries = MagicMock(return_value=[])
...
async def test_ensure_controller_exists_naming(hass, profile_service):
    ...
    await profile_service._ensure_controller_exists(
        "cronostar_thermostat_living_room_", "thermostat", meta
    )
...
async def test_get_profile_data_fallbacks(hass, profile_service, mock_storage_manager):
    ...
    mock_storage_manager.get_cached_containers = AsyncMock(
        return_value=[
            ("file.json", {"meta": {}, "profiles": {"Comfort": {"schedule": []}}})
        ]
    )
...
@pytest.mark.anyio
async def test_get_profile_data_diagnostics(
    ...
    mock_storage_manager.get_cached_containers = AsyncMock(
        side_effect=[
            ...
            [
                (
                    "f1.json",
                    {"meta": {"preset_type": "thermostat"}, "profiles": {"P1": {}}},
                )
            ],
        ]
    )
...
@pytest.mark.anyio
async def test_async_update_profile_selectors(
    hass, profile_service, mock_storage_manager
):
    """Test updating profile selectors."""
    mock_storage_manager.list_profiles = AsyncMock(return_value=["f1.json"])
    mock_storage_manager.load_profile_cached = AsyncMock(
        return_value={"meta": {"global_prefix": "prefix_"}, "profiles": {"P1": {}}}
    )
...
@pytest.mark.anyio
async def test_async_update_profile_selectors_error(
    hass, profile_service, mock_storage_manager
):
