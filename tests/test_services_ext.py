...
async def test_ensure_controller_exists_already_exists(hass, profile_service):
    ...
    hass.config_entries.async_entries = MagicMock(
        return_value=[MagicMock(data={"global_prefix": "exists_"})]
    )
...
@pytest.mark.anyio
async def test_get_profile_data_default_comfort_fallbacks(
...
@pytest.mark.anyio
async def test_profile_service_save_exception(
    hass, profile_service, mock_storage_manager
):
