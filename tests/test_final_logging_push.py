...
async def test_coordinator_logging_more(hass):
    ...
    hass.data[DOMAIN]["storage_manager"].list_profiles = AsyncMock(
        return_value=["f1.json"]
    )
    hass.data[DOMAIN]["storage_manager"].load_profile_cached = AsyncMock(
        return_value={"profiles": {"Default": {"schedule": []}}}
    )
...
async def test_coordinator_interpolate_more(hass):
    ...
    schedule = [{"time": "08:00", "value": 20.0}, {"time": "08:00", "value": 20.0}]
    # Line 343: t2 == t1
