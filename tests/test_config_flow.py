...
import pytest
from custom_components.cronostar.const import CONF_LOGGING_ENABLED, DOMAIN
...
async def test_config_flow_user_step(hass):
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], user_input={CONF_LOGGING_ENABLED: True}
    )
...
async def test_config_flow_create_controller(hass):
    ...
    result = await flow.async_step_create_controller(
        user_input={
            "name": "Test Room",
            "preset": "thermostat",
