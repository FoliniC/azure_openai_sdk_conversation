...
from custom_components.cronostar.const import CONF_LOGGING_ENABLED
from custom_components.cronostar.select import (
    CronoStarProfileSelect,
)
from custom_components.cronostar.select import (
    async_setup_entry as async_setup_select,
)
from custom_components.cronostar.sensor import (
    CronoStarCurrentSensor,
)
from custom_components.cronostar.sensor import (
    async_setup_entry as async_setup_sensor,
)
from custom_components.cronostar.setup import (
    _preload_profile_cache,
    _setup_static_resources,
...
from custom_components.cronostar.switch import (
    CronoStarEnabledSwitch,
)
from custom_components.cronostar.switch import (
...
async def test_setup_integration_failures(hass):
    ...
    with patch(
        "custom_components.cronostar.setup.validate_environment", return_value=False
    ):
        assert await async_setup_integration(hass, {}) is False

    with (
        patch(
            "custom_components.cronostar.setup.validate_environment", return_value=True
        ),
        patch(
            "custom_components.cronostar.setup._setup_static_resources",
            return_value=False,
        ),
    ):
        assert await async_setup_integration(hass, {}) is False
...
async def test_setup_events_startup(hass):
    ...
    assert hass.data["cronostar"][
        "profile_service"
    ].async_update_profile_selectors.called
...
async def test_validators_exceptions(hass):
    ...
    with patch(
        "custom_components.cronostar.setup.validators.Path.exists",
    ...
    with patch(
        "custom_components.cronostar.setup.validators.Path.is_dir", return_value=False
    ):
    ...
    with patch(
        "custom_components.cronostar.setup.validators.Path.touch",
        ...
        with (
            ...
            patch(
                "custom_components.cronostar.setup.validators.Path.is_dir",
                return_value=True,
            ),
        ):
            assert _check_profiles_directory(hass) is False
...
async def test_setup_static_resources_edge_cases(hass):
    ...
    with (
        patch("custom_components.cronostar.setup.Path.exists", return_value=True),
        patch(
            "custom_components.cronostar.setup.async_get_integration",
            side_effect=Exception("int error"),
        ),
    ):
        assert await _setup_static_resources(hass) is False
