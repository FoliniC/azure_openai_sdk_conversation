...
async def test_coordinator_storage_fallback(hass):
    ...
    with patch("custom_components.cronostar.coordinator.StorageManager") as mock_sm:
        _ = CronoStarCoordinator(hass, entry)
...
@pytest.mark.anyio
async def test_async_initialize_no_profiles(
...
@pytest.mark.anyio
async def test_async_refresh_profiles_no_files(
    hass, mock_coordinator, mock_storage_manager
):
...
@pytest.mark.anyio
async def test_async_refresh_profiles_exception(
    hass, mock_coordinator, mock_storage_manager
):
...
@pytest.mark.anyio
async def test_apply_schedule_empty_container(
    hass, mock_coordinator, mock_storage_manager
):
...
async def test_interpolate_schedule_invalid_points(mock_coordinator):
    ...
    schedule = [{"time": "invalid", "value": 20}, {"time": "08:00", "value": None}]
