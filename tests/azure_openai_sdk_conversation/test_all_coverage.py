...
from custom_components.azure_openai_sdk_conversation.const import CONF_API_BASE, CONF_CHAT_MODEL, DOMAIN
from custom_components.azure_openai_sdk_conversation.context.conversation_memory import ConversationMemoryManager
from custom_components.azure_openai_sdk_conversation.context.system_prompt import SystemPromptBuilder
...
def agent(hass, mock_config):
    ...
    with patch("custom_components.azure_openai_sdk_conversation.core.agent.ChatClient"), \
         patch("custom_components.azure_openai_sdk_conversation.core.agent.LocalIntentHandler"), \
         patch("custom_components.azure_openai_sdk_conversation.core.agent.SystemPromptBuilder"), \
         patch("custom_components.azure_openai_sdk_conversation.core.agent.ConversationMemoryManager"):
...
async def test_agent_process_success_llm(hass, mock_config):
    ...
    with patch("custom_components.azure_openai_sdk_conversation.core.agent.ChatClient") as MockClient, \
         patch("custom_components.azure_openai_sdk_conversation.core.agent.LocalIntentHandler") as MockLocal, \
         patch("custom_components.azure_openai_sdk_conversation.core.agent.SystemPromptBuilder") as MockBuilder, \
         patch("custom_components.azure_openai_sdk_conversation.core.agent.ConversationMemoryManager") as MockMem, \
         patch("homeassistant.components.conversation.get_agent_manager") as mock_get_manager:
...
async def test_agent_local_intent_fallback(hass, mock_config):
    ...
    with patch("custom_components.azure_openai_sdk_conversation.core.agent.ChatClient") as MockClient, \
        ...
        mock_local_response.response.response_type = "action_done"  # Ensure success type
...
async def test_system_prompt_builder(hass, mock_config):
    ...
    with patch("custom_components.azure_openai_sdk_conversation.context.system_prompt.EntityCollector") as MockCollector:
        ...
        mock_collector.collect = AsyncMock(return_value=[
            {"entity_id": "light.living", "name": "Living", "state": "on", "area": "Living Room"}
        ])
...
async def test_stream_parser_basic():
    ...
    from custom_components.azure_openai_sdk_conversation.llm.stream_parser import SSEStreamParser
