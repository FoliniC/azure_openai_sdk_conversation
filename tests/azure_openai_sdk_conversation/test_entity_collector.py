...
from custom_components.azure_openai_sdk_conversation.context.entity_collector import EntityCollector
...
async def test_collect_entities(hass, collector):
    ...
    with patch("homeassistant.helpers.entity_registry.async_get", return_value=mock_ent_reg), \
        ...
        mock_ent_reg.async_get.side_effect = lambda eid: entry1 if eid == "light.living_room" else entry2
