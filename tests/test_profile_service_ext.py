...
@pytest.mark.anyio
async def test_get_profile_data_diagnostics_extended(
        ...
        side_effect=[
            ...
            [
                (
                    "f1.json",
                    {
                        "meta": {"preset_type": "thermostat", "global_prefix": "p1"},
                        "profiles": {"P1": {}},
                    },
                )
            ],  # available
    ...
    res = await profile_service.get_profile_data(
        "Missing", "thermostat", global_prefix="p1"
    )
    assert "available_in_storage" in res


@pytest.mark.anyio
async def test_register_card_entity_states_error(
    hass, profile_service, mock_storage_manager
):
...
@pytest.mark.anyio
async def test_save_profile_update_entry_fields(
    hass, profile_service, mock_storage_manager
):
    ...
    call.data = {
        ...
        "global_prefix": "p1_",
        "meta": {"target_entity": "new.target", "preset_type": "fan"},
    }
